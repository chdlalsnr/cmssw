				  ...
				  const MuonDDDConstants& muonConstants )
{
  ...
  while (doSuper){
    ...
    if (fv.logicalPart().name().name() == "ME0Box") {
      // getting detId from etaPartitions
      fv.firstChild(); fv.firstChild();
      MuonDDDNumbering mdddnumCh(muonConstants);
      ME0NumberingScheme me0NumCh(muonConstants); 
      int rawId = me0NumCh.baseNumberToUnitNumber(mdddnumCh.geoHistoryToBaseNumber(fv.geoHistory())); //****************PROBLEM!!******************

      GEMDetId detId = GEMDetId(rawId);
      fv.parent(); fv.parent(); // back to chambers
      ...
      // only make superChambers when doing layer 1
      if (detId.layer() == 1) {
        GEMSuperChamber *gemSuperChamber = buildSuperChamber_m(fv, detId); 
      ...
      while (doChambers) {
        ...
        GEMChamber *chamber = buildChamber_m(fv, detId);
        ...
        while (doEtaPart) {
          ...
          GEMEtaPartition* etaPart = (GEMEtaPartition*) buildEtaPartition(fv, detId);
          chamber->add(etaPart);
      ...
      int rawid = gemNumCh.baseNumberToUnitNumber(mdddnumCh.geoHistoryToBaseNumber(fv.geoHistory()));
      GEMDetId detId = GEMDetId(rawid);
      fv.parent(); fv.parent();
      ...
      GEMChamber *chamber = buildChamber(fv, detId);
      bool doChambers = fv.firstChild();
      ...
      fv.parent();
      doSuper = fv.nextSibling();
      if (!loopExecuted) delete chamber;
    }
  }
  ...
  for (int re = -1; re <= 1; re = re+2) {
    ...
    for (int st = 0; st <= GEMDetId::maxStationId; st++) {
      ...
      if (st == 0) { std::string name("ME" + sign + std::to_string(st)); station->setName(name); }
      else { std::string name("GE" + sign + std::to_string(st) + "/1"); station->setName(name); }
        ...
        for (auto superChamber : superChambers) {
          ...
          if (st == 0) {
            for (int la = 1; la <= 6; la++) {
              superChamber->add(theGeometry.chamber(GEMDetId(re, ri, st, la, detId.chamber(), 0)));
            }
          }
          else {
            superChamber->add(theGeometry.chamber(GEMDetId(re, ri, st, 1, detId.chamber(), 0)));
            superChamber->add(theGeometry.chamber(GEMDetId(re, ri, st, 2, detId.chamber(), 0)));
          }
...
GEMSuperChamber* GEMGeometryBuilderFromDDD::buildSuperChamber_m(DDFilteredView& fv, GEMDetId detId) const {
...
GEMChamber* GEMGeometryBuilderFromDDD::buildChamber_m(DDFilteredView& fv, GEMDetId detId) const {
  
  LogTrace("GEMGeometryBuilderFromDDD") << "buildChamber " << fv.logicalPart().name().name() << " " << detId << std::endl;
  DDBooleanSolid solid = (DDBooleanSolid)(fv.logicalPart().solid());
  std::vector<double> dpar = solid.parameters();
  
  double L = dpar[0]/cm;// length is along local Y
  double b = dpar[3]/cm;// thickness is long local Z
  double B = dpar[4]/cm;// bottom width is along local X
  double t = dpar[8]/cm;// top width is along local X
...
GEMGeometryBuilderFromDDD::RCPBoundPlane GEMGeometryBuilderFromDDD::boundPlane(const DDFilteredView& fv, Bounds* bounds, bool isOddChamber) const {
